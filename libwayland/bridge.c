...
static void
_xdg_shell_ping(void *data, struct xdg_wm_base *shell, uint32_t serial)
{
	xdgcallback_handle_shell_ping((uintptr_t)(data), shell, serial);
}
...
{
    xdg_wm_base_add_listener((struct xdg_wm_base *)shell, &_xdg_shell_listener, data);
}

static void
_handle_xdg_surface_configure(void *data, struct xdg_surface *surface,
			     uint32_t serial)
{
	xdgcallback_handle_surface_configure((uintptr_t)data, surface, serial);
...
int
_xdg_surface_add_listener(void *xdg_surface, void *data)
{
    return xdg_surface_add_listener((struct xdg_surface *)xdg_surface, &_xdg_surface_listener, data);
}
			  ...
			  struct wl_array *states)
{
	...
	xdgcallback_handle_toplevel_configure((uintptr_t)(data), xdg_toplevel, width, height, pos,len);
...
_handle_xdg_toplevel_close(void *data,
		      struct xdg_toplevel *xdg_toplevel)
{
	xdgcallback_handle_toplevel_close((uintptr_t)(data), xdg_toplevel);
...
int
_xdg_toplevel_add_listener(void *xdg_toplevel, void *data)
{
	return xdg_toplevel_add_listener((struct xdg_toplevel *)xdg_toplevel,
			      &_xdg_toplevel_listener, data);
}

void
_xdg_toplevel_set_title(void *xdg_toplevel, void *titl, int len)
{
	char *title = (char*) titl;
	if ((len > 0) && (title[len-1] == 0)) {
		// now it is null terminated
		wl_proxy_marshal((struct wl_proxy *) xdg_toplevel,
				 XDG_TOPLEVEL_SET_TITLE, &title[0]);
	}
}
...
_handle_output_geometry(void *data,
			...
			int transform)
{
	char *omake = (char *) make;
	char *omodel = (char *) model;
